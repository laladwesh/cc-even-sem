...
const [badges, setBadges] = useState([]);
    ...
    (async () => {
      ...
      try {
        ...
        const [cRes, pRes] = await Promise.all([
          ...
          axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/admin/badges/allocations`, { headers:{ Authorization:`Bearer ${token}` } }),
        ]);
        setCourses(cRes.data.courses || cRes.data);
        setProjects(pRes.data.projects || pRes.data);
        setBadges(cRes[2]?.data.badges || pRes[2]?.data.badges || []);
      } catch (err) {
  ...
  return (
    <div className="px-16 py-24 bg-background space-y-8">
      {/* Action Buttons */}
      <div className="flex flex-wrap py-4 gap-4">
        <button
          onClick={() => setShowCourseModal(true)}
          className="inline-flex items-center px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-500 hover:from-purple-700 hover:to-indigo-600 text-white rounded-lg shadow transition-transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-purple-300"
        >
          <svg
        ...
        <button
          ...
          className="inline-flex items-center px-8 py-3 bg-gradient-to-r from-green-600 to-teal-500 hover:from-green-700 hover:to-teal-600 text-white rounded-lg shadow transition-transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-green-300"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-5 h-5 mr-2"
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="bg-white self-start w-1/3 rounded-xl p-8 mb-8" >
            <h2 className="text-3xl text-primary py-8 font-semibold mb-4">All Courses</h2>
          ...
          `}
                    >
                      ...
                      <td className="px-6 py-8">
                        <span className="inline-block px-2 py-2 bg-indigo-100 text-indigo-800 text-xs font-semibold uppercase rounded-2xl">
          ...
          {/* Projects Management */}
          <section className="bg-white w-1/3 self-start rounded-xl px-8 mb-8 py-8">
            <h2 className="text-3xl text-primary py-8 font-semibold mb-4">All Projects</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full table-auto text-sm">
                <thead>
                  <tr className="bg-gray-100 text-left">
          ...
          <section className="bg-white self-start rounded-xl w-1/3 p-8 mb-8" >
            <h2 className="text-3xl text-primary py-8 font-semibold mb-4">Badge Allocation</h2>
             {badges.length === 0 ? (
